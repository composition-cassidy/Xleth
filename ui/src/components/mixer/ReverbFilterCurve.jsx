// ─── ReverbFilterCurve.jsx ────────────────────────────────────────────────────
// Interactive wet-path filter display — replaces the old HI CUT / LO CUT knob
// pair.
//
// The accent curve is the real response of the engine's reverb filter: every
// backend applies a one-pole low-pass at `hicut` in series with a one-pole
// high-pass at `locut` (coefficients exp(-2*pi*f/sr) in XlethReverbEffect.h),
// which is the same first-order LP×HP topology the Delay's feedback filter
// uses — hence the shared response maths in mixerFilterMath.js.
//
// The RANGES are the reverb's own and differ from the Delay's: locut tops out
// at 500 Hz, not 2 kHz.
//
// Interaction contract (identical to DelayFilterCurve, deliberately):
...
const F_MIN = REVERB_FILTER_F_MIN
const F_MAX = REVERB_FILTER_F_MAX

export default function ReverbFilterCurve({ loHz, hiHz, onPreview, onCommit }) {
  ...
  const paint = useCallback(() => {
    ...
    if (!canvas) return
    const { cssW, cssH, dpr } = syncCanvasSize(canvas, 240, 74)
    const ctx = canvas.getContext('2d', { alpha: false })
    if (!ctx) return
    ctx.setTransform(dpr, 0, 0, dpr, 0, 0)

    const th = readMixerCanvasTheme(canvas)
    const lo = clamp(loHz, REVERB_LOCUT_MIN, REVERB_LOCUT_MAX)
    const hi = clamp(hiHz, REVERB_HICUT_MIN, REVERB_HICUT_MAX)
    ...
    const xOf = (f) => padL + freqToNorm(f, F_MIN, F_MAX) * plotW
    ...
    for (let x = 0; x <= plotW; x += 2) {
      const f = normToFreq(x / plotW, F_MIN, F_MAX)
    ...
    // Readouts pinned to the plot corners rather than tracking their handles —
    // the handles converge mid-band and a handle-following label collides with
    // its partner there.
    ctx.font = MONO
    ctx.textBaseline = 'top'
    ctx.textAlign = 'left'
  ...
  const valueFor = useCallback((handle, norm) => {
    const freq = normToFreq(norm, F_MIN, F_MAX)
    return handle === 'lo'
      ? clamp(freq, REVERB_LOCUT_MIN, REVERB_LOCUT_MAX)
      : clamp(freq, REVERB_HICUT_MIN, REVERB_HICUT_MAX)
  }, [])
  ...
  const handlePointerDown = useCallback((e) => {
    ...
    const handle = pickFilterHandle(norm, lo, hi, 1, F_MIN, F_MAX)
    if (!handle) return
    e.preventDefault()
    try { e.currentTarget.setPointerCapture(e.pointerId) } catch { /* older engines */ }
    const handleNorm = freqToNorm(handle === 'lo' ? lo : hi, F_MIN, F_MAX)
  ...
  const handlePointerMove = useCallback((e) => {
    ...
    if (!drag) {
      const { lo, hi } = liveRef.current
      setHover(pickFilterHandle(norm, lo, hi, 0.05, F_MIN, F_MAX))
      return
    }
    ...
    onPreview?.(drag.handle === 'lo' ? 'locut' : 'hicut', next)
  ...
  const handlePointerUp = useCallback(() => {
    ...
    setActive(null)
    const { lo, hi } = liveRef.current
    onCommit?.(drag.handle === 'lo' ? 'locut' : 'hicut', drag.handle === 'lo' ? lo : hi)
  }, [onCommit])

  return (
    <canvas
      ref={canvasRef}
      className="reverb-filter-canvas"
      onPointerDown={handlePointerDown}
      onPointerMove={handlePointerMove}
      onPointerUp={handlePointerUp}
      onPointerCancel={handlePointerUp}
      onPointerLeave={() => { if (!dragRef.current) setHover(null) }}
      title="Drag the corner handles to set the reverb's wet filter band"
