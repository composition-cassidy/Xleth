...
describe('effectChainStore FX mode safety gate', () => {
  ...
  // The Envelope runtime parameter drive that used to be tested here is GONE: the
  // Envelope Controller is evaluated in the engine now, on the audio thread, against
  // the authoritative transport position. Its behavior is covered by
  // engine/test/test_envelope_parameter_modulation.cpp (pure evaluator, mapping, skip
  // semantics) and engine/test/test_envelope_modulation_engine.cpp (gates from the real
  // Timeline, plus play-start / loop wrap / seek / stop through processBlock).
  // The store keeps only the AUTHORING actions, tested further below.
