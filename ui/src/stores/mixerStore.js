...
const useMixerStore = create((set, get) => ({
  tracks: {},       // { [trackId]: { id, name, volume, pan, spread, muted, solo, type } }
  trackOrder: [],   // [id, ...]
  master: { volume: 1.0 },
  visible: false,
  ...
  syncFromTimeline: (list) => {
    ...
    set(s => {
      ...
      for (const t of list) {
        ...
        if (existing) {
          tracks[t.id] = { ...existing, name: t.name, muted: t.muted, solo: t.solo, type: t.type }
        } else {
          tracks[t.id] = {
            id: t.id, name: t.name,
            volume: t.volume ?? 1.0, pan: t.pan ?? 0.0, spread: t.stereoSpread ?? 1.0,
            muted: t.muted, solo: t.solo, type: t.type,
          }
          if (!peaksSnapshot.tracks[t.id]) {
            peaksSnapshot.tracks[t.id] = newPeakEntry()
          }
