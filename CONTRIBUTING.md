...
## Preview-performance regression gate

`bridge/test_perf_regression.js` guards against a specific, previously-shipped class of bug: `SyncManager::videoTick()` performing blocking `decoder->seekAndDecode()` calls in the bridge (`.node`/`XLETH_CORE_ONLY`) build whose decoded frame nothing ever consumed there (`compositor_` is always `nullptr` in that build — only the full-engine executable wires a real one). Each call cost 100–330ms on scattered seeks, blocking the frame-pacing thread under `syncEventsMutex`, and collapsed a real project's preview from 60fps to ~17fps before it was fixed in `3db1b33`. It runs as part of `bridge`'s `npm test` (picked up automatically, like every `test_*.js` script) and as a pre-push gate in `push.bat`.

**What it does.** It builds a synthetic project entirely through the addon's own RPCs at gate time — no checked-in fixture, no user files, no network: a short test clip generated with the vendored FFmpeg (`vendor/ffmpeg/bin/ffmpeg.exe`, `testsrc2` source filter), 30 tracks with a clip spanning the whole measured playback window (comparable to the "few concurrently active" shape of the real project this bug was found on) plus enough short, off-window filler clips to reach ~1000+ total registered clips (comparable to that project's 1738). It then plays the project and samples `diag_getVisualPreviewDiagnostic()` — the engine's own preview diagnostics, not a hand-rolled timer — at high frequency.

**What it asserts:**

- **Hard invariant — `counters.syncManagerDecodeCount` stays exactly 0 for the whole run.** This is the deterministic signal: it is incremented at the exact call site of the historical bug (`SyncManager::getDecodeAttemptCount()`, mirroring `decodeTimeSamples_.size()`), which only exists in a build where the guarding `#ifndef XLETH_CORE_ONLY` around that decode has been removed or bypassed. Any non-zero value means the bug (or something structurally identical to it) is back, regardless of how fast the fixture's own test video happens to decode.
- **`videoTick` timing budget — p95 ≤ 5ms, mean ≤ 2ms**, measured from `counters.lastVideoTickUs`/`avgVideoTickUs` (added alongside the decode counter specifically to time the `syncManager->videoTick()` call itself — the existing `avgTickUs`/`lastTickUs` fields time a *later*, separate stage, the compositor pipeline, which only starts after `videoTick()` has already returned). Generous headroom above the fixed baseline (0.02–0.07ms measured) so slower dev machines don't false-positive; still well below the 15–141ms range the original bug produced. This budget is secondary to the hard invariant above — on a fast, simple fixture video a resurrected bug's decode can complete well under 5ms even though it is still pure dead work, which is exactly why the invariant exists as a separate, load-independent check.
- **Sanity (non-vacuous run)** — steady-state delivered FPS ≥ 90% of the 60fps cap, and `lastCellCount`/`requestCount` were both observed > 0 at least once, so the gate cannot pass by silently measuring nothing (e.g. a GPU compositor that failed to initialize, or a fixture that never actually played).

**When it skips (exit 0, not a failure):** no usable D3D11 device (`compositorReady === false` — preview FPS budgets aren't meaningful without a real compositor), or `vendor/ffmpeg/bin/ffmpeg.exe` is missing.

**If it fails:** the printed summary table shows per-stage timings, the `videoTick` p50/p95/max, the decode count, and delivered FPS — enough to diagnose without local repro. A non-zero `syncManagerDecodeCount` means something is calling `decoder->seekAndDecode()` from `SyncManager::videoTick()` again in the bridge build; check that the guard around SyncManager.cpp steps 4d–4h is still `#ifndef XLETH_CORE_ONLY` and still contiguous through 4f–4h (compositor upload), matching commit `3db1b33`. A `videoTick` budget failure with `syncManagerDecodeCount === 0` points at a *different* per-tick cost added to `videoTick()` (the drains, the beat-window loop itself, or something new) rather than a repeat of this specific bug.

**Skipping the gate for an emergency push:** `set XLETH_SKIP_PERF_GATE=1 && push.bat`. This skips only the pre-push check, not the commit — use it when you need to push urgently and will follow up with a real fix, not as a routine workaround for a budget the gate correctly caught.
